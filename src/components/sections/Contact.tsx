...
import { Send, Terminal, Mail, Phone, User, MessageSquare, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const Contact = ({ language }: ContactSectionProps) => {
  ...
  const validateForm = useCallback((form: HTMLFormElement): boolean => {
    ...
    if (!name || name.length < 2) {
      errors.name = language === 'en' ? 'Name is too short' : language === 'pt' ? 'Nome muito curto' : language === 'it' ? 'Nome troppo corto' : 'El nombre es muy corto';
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!email || !emailRegex.test(email)) {
      errors.email = language === 'en' ? 'Enter a valid email' : language === 'pt' ? 'Insira um email válido' : language === 'it' ? 'Inserisci una email valida' : 'Ingresa un email válido';
    }

    if (!message || message.length < 10) {
      errors.message = language === 'en' ? 'Message is too short' : language === 'pt' ? 'Mensagem muito curta' : language === 'it' ? 'Messaggio troppo corto' : 'El mensaje es muy corto';
    }
  ...
  if (state.succeeded || (showCooldown && timeLeft > 0)) {
    return (
      <section id="contacto" className="py-16 sm:py-20" aria-label="Contacto">
    ...
    <section id="contacto" className="py-16 sm:py-20" aria-label="Contacto">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <div className="w-20 sm:w-24 h-1 bg-gradient-to-r from-blue-600 to-purple-600 mx-auto rounded-full mt-4" />
        </motion.div>
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-6 sm:gap-8">
            ...
            <div className="p-4 sm:p-6 font-mono text-xs sm:text-sm space-y-2 sm:space-y-3">
              ...
              <div className="mt-4 sm:mt-6 space-y-2 sm:space-y-3">
                <div className="flex items-center gap-2 sm:gap-3 bg-gray-800 rounded-lg p-2.5 sm:p-3">
                  <Mail size={16} className="text-blue-400 sm:w-[18px] sm:h-[18px]" aria-hidden="true" />
                  <a href={`mailto:${contactInfo.email}`} className="text-xs sm:text-sm text-gray-300 hover:text-white transition-colors break-all">
                    {contactInfo.email}
                  </a>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 bg-gray-800 rounded-lg p-2.5 sm:p-3">
                  <Phone size={16} className="text-green-400 sm:w-[18px] sm:h-[18px]" aria-hidden="true" />
                  <a href={`tel:${contactInfo.phone}`} className="text-xs sm:text-sm text-gray-300 hover:text-white transition-colors">
                    {contactInfo.phone}
                  </a>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 bg-gray-800 rounded-lg p-2.5 sm:p-3">
                  <Terminal size={16} className="text-purple-400 sm:w-[18px] sm:h-[18px]" aria-hidden="true" />
          ...
          >
            <form onSubmit={onSubmit} className="space-y-4 sm:space-y-5" noValidate>
              ...
              {/* Name */}
              <div>
                <label htmlFor="contact-name" className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-900 dark:text-white mb-2">
                  <User size={14} className="text-blue-600 dark:text-blue-400 sm:w-4 sm:h-4" aria-hidden="true" />
                ...
                {formErrors.name && (
                  <p id="name-error" className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={12} aria-hidden="true" /> {formErrors.name}
                  </p>
                )}
              ...
              {/* Email */}
              <div>
                <label htmlFor="contact-email" className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-900 dark:text-white mb-2">
                  <Mail size={14} className="text-blue-600 dark:text-blue-400 sm:w-4 sm:h-4" aria-hidden="true" />
                ...
                {formErrors.email && (
                  <p id="email-error" className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={12} aria-hidden="true" /> {formErrors.email}
                  </p>
                )}
              ...
              {/* Phone (Optional) */}
              <div>
                <label htmlFor="contact-phone" className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-900 dark:text-white mb-2">
                  <Phone size={14} className="text-blue-600 dark:text-blue-400 sm:w-4 sm:h-4" aria-hidden="true" />
              ...
              {/* Message */}
              <div>
                <label htmlFor="contact-message" className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-900 dark:text-white mb-2">
                  <MessageSquare size={14} className="text-blue-600 dark:text-blue-400 sm:w-4 sm:h-4" aria-hidden="true" />
                ...
                {formErrors.message && (
                  <p id="message-error" className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={12} aria-hidden="true" /> {formErrors.message}
                  </p>
                )}
